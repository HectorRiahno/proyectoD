// Datos de muestra de pacientes para desarrollo local
export const pacientesList = [
  {
    id: 1,
    nombre: "Carlos López Martínez",
    documento: "300400500",
    edad: 39,
    telefono: "+57 300 123 4567",
    email: "carlos.lopez@email.com",
    diagnostico: "Control de hipertensión",
    ultimaVisita: "2024-10-20"
  },
  {
    id: 2,
    nombre: "Ana Rodríguez Silva",
    documento: "400500600",
    edad: 34,
    telefono: "+57 310 987 6543",
    email: "ana.rodriguez@email.com",
    diagnostico: "Seguimiento diabetes tipo 2",
    ultimaVisita: "2024-10-18"
  },
  {
    id: 3,
    nombre: "Luis Martínez Torres",
    documento: "500600700",
    edad: 46,
    telefono: "+57 320 456 7890",
    email: "luis.martinez@email.com",
    diagnostico: "Consulta general",
    ultimaVisita: "2024-10-15"
  },
  {
    id: 4,
    nombre: "María Fernández García",
    documento: "100200300",
    edad: 28,
    telefono: "+57 315 234 5678",
    email: "maria.fernandez@email.com",
    diagnostico: "Control prenatal",
    ultimaVisita: "2024-10-22"
  },
  {
    id: 5,
    nombre: "Pedro Gómez Ruiz",
    documento: "200300400",
    edad: 52,
    telefono: "+57 301 876 5432",
    email: "pedro.gomez@email.com",
    diagnostico: "Chequeo cardiológico",
    ultimaVisita: "2024-10-19"
  },
  {
    id: 6,
    nombre: "Laura Sánchez Díaz",
    documento: "600700800",
    edad: 41,
    telefono: "+57 318 654 3210",
    email: "laura.sanchez@email.com",
    diagnostico: "Control endocrinología",
    ultimaVisita: "2024-10-21"
  }
];

// Función para buscar pacientes por nombre
export function buscarPacientes(termino) {
  const term = termino.toLowerCase();
  return pacientesList.filter(p => 
    p.nombre.toLowerCase().includes(term) ||
    p.documento.includes(termino)
  );
}

// Función para obtener paciente por ID
export function getPacienteById(id) {
  return pacientesList.find(p => p.id === id);
}